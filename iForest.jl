...
combine(groupby(glass, :label), nrow)
...
# ╔═╡ e9b7f863-7edc-4ddb-abc2-0981c572a640
color_by_label

# ╔═╡ d3422028-5f9d-4518-a48e-19a690570fa1
begin
	local fig = Figure()
	for (i, (merge)) in enumerate(true)
		local ax = Axis(fig[1,1], xlabel="Sodium", ylabel = "Silicon")
		if color_by_label
			for i in 1:nrow(glass)
				if glass[:, "label"][i] == "normal"
					scatter!(glass[:, "Na"][i], glass[:, "Si"][i],
			...
			axislegend(position=:rb, merge=merge)
		else
			scatter!(glass[:, "Na"], glass[:, "Si"], color=:black)
		end
	end
	fig
end

# ╔═╡ 79c478fb-de10-4fab-8c56-bf09a17f185e
glass
...
# ╔═╡ 95118964-7180-466c-8265-593ce3b99eba
glass

# ╔═╡ 15ded17b-0b0b-4472-b6a6-ec517b1731a9
glass[glass[:, "label"].=="normal", :]
...
Yes the algorithm prediction are consistent with the raw data plot
"

# ╔═╡ 1cb15395-d9b1-4198-84f6-3e26b3dfe294
X_test =[14.8 73.1; 12.8 73.15]

# ╔═╡ 9a8daaec-c80c-494f-8bca-df72d2b44868
clf.predict(X_test)
...
# ╔═╡ 103c5f63-55f0-4e14-993b-f6fcc3e0a72d
X_tuple = (collect(12:0.01:18), collect(71:0.01:77))

# ╔═╡ 21b41acb-5d14-463b-a49d-5ae579d91352
begin
	X1, X2 = X_tuple
	...
	for i in 1:length(X1)
		for j in 1:length(X2)
			...
			feature_matrix[i, j] = clf.decision_function(matrix_space)[1]
		end
	end
end

# ╔═╡ 9ec455b4-e8ea-456a-8697-c356b83be82a
feature_matrix

# ╔═╡ 91fbdaad-19b5-4120-9b16-e9c18689425f


# ╔═╡ 00967516-a826-4cba-8222-4cf7d4404738
function plot_anomaly(X, feature_matrix=feature_matrix)
	local fig = Figure()
	local ax = Axis(fig[1, 1], xlabel="Sodium", ylabel="Silicon", title="Anomaly Score Grid")
	
	normal = glass[glass[:, "label"].=="normal", :]
	anomaly = glass[glass[:, "label"].=="anomaly", :]
	X1, X2= X
	...
	scatter!(normal[:, "Na"], normal[:, "Si"], color=colors["normal"])
	scatter!(anomaly[:, "Na"], anomaly[:, "Si"], color=colors["anomaly"])
	
	fig
end

# ╔═╡ d9f69ba2-f3fd-4180-9cdb-a7c6c015e9d7
...
begin
	y_pred = Int64.(anomaly_scores .> 0)
	y = Int64.(glass[:, "label"].=="normal")
end
...
viz_confusion_matrix(matrix, ["anomaly", "normal"])
...
md"""
* There are 4 false alarms and there are 6 anomaly that went undetected
"""
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "dec33cb420f56c74d467c661dfd17fdeefaee9b9"
